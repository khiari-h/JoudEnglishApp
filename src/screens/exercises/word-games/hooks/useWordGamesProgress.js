...
const useWordGamesProgress = (level) => {
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        ...
        const savedCompletedGamesJson = await AsyncStorage.getItem(
          COMPLETED_GAMES_KEY
        );
        ...
        const savedScores = savedScoresJson ? JSON.parse(savedScoresJson) : [];
  ...
  const markGameAsCompleted = useCallback(
    async (gameIndex, score, maxScore) => {
      try {

...
const Select = ({
  ...
  multipleSelectionLabel = (count) => `${count} option${count !== 1 ? 's' : ''} sélectionnée${count !== 1 ? 's' : ''}`,
  ...
  const shouldShowPlaceholder = !selectedValue || (multiple && Array.isArray(selectedValue) && selectedValue.length === 0);
  ...
  const renderOptionItem = ({ item }) => {
    ...
    if (renderOption) {
      return renderOption({ item, selected, onSelect: () => handleSelectOption(item) });
    }
    ...
    return (
      ...
      >
        ...
        {selected && (
          <Ionicons name="checkmark" size={18} color="#5E60CE" />
        )}
      </TouchableOpacity>
    );
  };
  ...
  const getDropdownPosition = () => {
    ...
    if (dropdownPosition === 'top' || (dropdownPosition === 'bottom' && spaceBelow < 200 && spaceAbove > spaceBelow)) {

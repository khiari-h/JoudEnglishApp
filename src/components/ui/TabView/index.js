...
const TabView = ({
  ...
  useEffect(() => {
    ...
    if (variant === 'scrollable' && tabScrollViewRef.current) {
      tabScrollViewRef.current.scrollTo({
        x: activeIndex * (tabWidth || 100) - windowWidth / 2 + (tabWidth || 100) / 2,
        animated: true,
      });
    }
  ...
  const getIndicatorTranslate = () => {
    ...
    return indicatorAnimation.interpolate({
      inputRange: tabs.map((_, i) => i),
      outputRange: tabs.map((_, i) => i * (tabWidth || windowWidth / tabs.length)),
      extrapolate: 'clamp',
    });
  };
      ...
      return (
        ...
        >
          {tabs.map((tab, index) => (
            <View
              key={index}
              style={[styles.scene, { width: contentWidth }]}
            >
              {(!lazy || loadedTabs.includes(index)) && renderScene({ route: tab, index })}
            </View>
          ))}
        </ScrollView>
      );
